...
from torch_mimicry.modules.resblocks import DBlock as DBlock  # type: ignore
from torch_mimicry.modules.resblocks import DBlockOptimized, GBlock  # type: ignore


class GBlock(GBlock):  # type: ignore
    r"""
...
class DBlock(DBlock):  # type: ignore
    """
    ...
    def __init__(
        ...
        if spectral_norm and sn is not None:
            self.c1.weight_pad_to = self.c2.weight_pad_to = (im_size, im_size)
            self.c1 = sn(self.c1)
            self.c2 = sn(self.c2)
            if self.learnable_sc:
                self.c_sc.weight_pad_to = (im_size, im_size)
                self.c_sc = sn(self.c_sc)


class DBlockOptimized(DBlockOptimized):  # type: ignore
    """
    Optimized residual block for discriminator. This is used as the first residual
    block, where there is a definite downsampling involved. Follows the official SNGAN
    reference implementation in chainer.
