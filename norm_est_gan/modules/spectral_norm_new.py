...
class SpectralNorm:
    def __init__(self, upd_gamma_every=1000, n_samp=10, upd_est_every=1, denom=None, default=False):
    ...
    def apply(self, module: nn.Module, name: str="weight") -> nn.Module:
        if self.default == True:
            ...
            NormEstimation.apply(module, name, self.upd_gamma_every, self.n_samp, self.upd_est_every, self.denom)
            return module
...
class NormEstimation:
    ...
    def __init__(self, name: str="weight", upd_gamma_every: int=1000, n_samp: int=10, 
                 upd_est_every: int=1, denom: Optional[float]=None):
        self.name = name
        self.upd_gamma_every = upd_gamma_every
        self.upd_est_every = upd_est_every
        self.n_samp = n_samp
    ...
    def estimate_norm(self, module: nn.Module):
        W = getattr(module, f"{self.name}_orig")
        samp = torch.randn([self.n_samp, W.shape[1]] + list(getattr(module, f'{self.name}_pad_to')), device=W.device)
        out = self.forward(module, samp)
        normsq = ((out ** 2).sum(list(range(len(out.shape)))[1:])).mean(0)
        estimate = normsq / (getattr(module, f'{self.name}_gamma') * getattr(module, f'{self.name}_ndim'))
        return estimate
    ...
    def compute_weight(self, module):
        ...
        if self.denom is not None:
            weight = weight / self.estimate_norm(module) ** .5 / self.denom
        return weight
    ...
    def apply(module: nn.Module, name, upd_gamma_every, n_samp, upd_est_every, denom):
        ...
        fn = NormEstimation(module, name, 0, upd_gamma_every, n_samp, upd_est_every, denom)
...
class SpectralNormLoadStateDictPreHook:
    ...
    def __call__(self, state_dict, prefix, local_metadata, strict,
        ...
        if version is None or version < 1:
            ...
            if version is None and all(weight_key + s in state_dict for s in ('_orig', '_gamma')) and \
